...
module MsUtils
  ...
  def gooddata_login(login, password) 
  ...
  def self.create_deleted(options={})
    ...
    fail "You have to provide base_dir if you have no data in es_dir" if MsUtils.is_empty_dir?(es_dir) && base_dir.nil?
    fail "You have to provide path to descriptor_file if you have no data in sf_dir" if MsUtils.is_empty_dir?(sf_dir) && descriptor_file.nil?

    MsUtils.es_ids(:password => gd_password, :login => gd_login, :es_name => ES_NAME, :base_dir => base_dir, :output_dir => es_dir, :pid => pid) if MsUtils.is_empty_dir?(es_dir)
    MsUtils.sf_ids(:password => SF_PASSWORD, :login => sf_login, :output_dir => sf_dir, :descriptor => descriptor_file) if MsUtils.is_empty_dir?(sf_dir)

    left_in_sf = []
    left_in_es = []
    mapping, left_in_sf, left_in_es = MsUtils.guess_mapping(sf_dir, es_dir) if mapping.nil?
    ...
    mapping.each do |map|
      ...
      MsUtils.diff_files(map[:file_es], map[:file_sf], output_file_name, timestamp)
    end

  end
  
  def is_empty_dir?(dir, pattern="*.csv")
  ...
  def guess_mapping(dir_sf, dir_es)
